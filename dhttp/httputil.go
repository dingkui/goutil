...
func DoHttp(method string, url string, bodys []byte, headers map[string]string, checkStatus func(string, int) error) ([]byte, error) {
	...
	if err != nil {
		dlog.Error("网络故障-1:url=%s,%s", url, err.Error())
	...
	if err != nil {
		dlog.Error("网络故障-2:url=%s,%s", url, err.Error())
	...
	if err != nil {
		dlog.Error("网络故障-3:url=%s,%s", url, err.Error())
...
func POSTAsJson(url string, param []byte, headers map[string]string) (*djson.JSON, error) {
	resBody, err := POST(url, param, headers, nil)
	if err != nil {
		return nil, err
	}
	return djson.NewJSON(resBody), nil
}
func Get(url string) ([]byte, error) {
	return GET(url, nil, nil)
}
func GetAsJson(url string) (*djson.JSON, error) {
...
func GETAsJson(url string, headers map[string]string) (*djson.JSON, error) {
...
func Get2File(dataFile, url string) (result bool) {
	...
	err = native.WriteAndSyncFile(dataFile, respBody, os.ModePerm)
	if err != nil {
		dlog.Error("保存文件失败：%s dataFile=%s", err.Error(), dataFile)
		return
	}
	result = true
	return
